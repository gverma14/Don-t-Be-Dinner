...
static const wchar_t *WINCLASS=L"MainWindowClass";
static const wchar_t *WINNAME=L"MainWindow";
...
void FsOpenWindow(int x0,int y0,int wid,int hei,int useDoubleBuffer)
{
	...
	if(0!=RegisterClassW(&wc))
	{
		...
		fsWin32Internal.hWnd=CreateWindowW(WINCLASS,WINNAME,WINSTYLE,x0,y0,wid,hei,NULL,NULL,inst,NULL);
...
static LONG WINAPI WindowFunc(HWND hWnd,UINT msg,WPARAM wp,LPARAM lp)
{
	...
	case WM_CREATE:
		...
		SetPixelFormat(fsWin32Internal.hDC,doubleBuffer);
...
static void SetPixelFormat(HDC hDC,int useDoubleBuffer)
{
